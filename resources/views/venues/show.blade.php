...
<div class="container my-4">
  <div class="card shadow-sm">
    <div class="card-body p-4">
      ...
      <h2 class="h5 mb-2">
        現在の混雑状況: <span id="currentAverageCongestion" class="text-primary fw-bold">
          {{ \App\Helpers\CongestionHelper::getText($venue->average_congestion) }}
        </span>
      </h2>

      <h3 class="h6 mt-4 mb-2">混雑状況を報告する</h3>
      ...
      <form method="POST" action="{{ route('venues.favorite.toggle', $venue) }}" class="mb-4">
        @csrf
        @if ($isFavorited)
        ...
        @else
          ...
          <button type="submit" class="btn btn-line">🔔 混雑状況が変わったらLINEで通知</button>
          @else
            <button type="button" class="btn btn-secondary" disabled>🔔 混雑状況が変わったらLINEで通知（準備中）</button>
          @endif
        @endif
      </form>
      ...
      <h3 class="h6 mt-4 mb-2">サ活を投稿する（写真付き口コミ）</h3>
      <form action="{{ route('venues.reviews.store', $venue) }}" method="POST" enctype="multipart/form-data" class="bg-light p-3 rounded shadow-sm">
        ...
        <div class="mb-2">
          <label class="form-label small">サ活ログ</label>
          <textarea name="comment" class="form-control form-control-sm" rows="3" minlength="5" maxlength="1000" required placeholder="例：ロウリュ最高でした。水風呂も広くて整いました。"></textarea>
        </div>
        <div class="mb-2">
      ...
      <h3 class="h6 mt-5 mb-3">サ活・口コミ</h3>
      <div id="reviewList">
        @forelse($venue->reviews as $review)
          <div class="card mb-3 bg-light">
            @if($review->photo_path)
              <img src="{{ \Illuminate\Support\Facades\Storage::url($review->photo_path) }}" class="card-img-top" style="max-height:320px;object-fit:cover;" alt="{{ $venue->name }}のサ活写真">
            @endif
            <div class="card-body">
